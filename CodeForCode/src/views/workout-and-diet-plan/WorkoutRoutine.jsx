        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-extrabold text-foreground tracking-tight">
              Workout <span className="text-teal-500 dark:text-teal-400">Routines</span>
            </h1>
          </div>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Create, manage, and customize dynamic workout routines for your gym members</p>
        </motion.div>
        ...
        >
          ...
          <section className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-foreground">Your Routines</h2>
            </div>
                    ...
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div>
                            <h3 className="font-bold text-xl text-foreground group-hover:text-teal-600 transition-colors">{r.name}</h3>
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                r.goal === 'Strength' ? 'bg-red-100 text-red-700' :
                                r.goal === 'Muscle' ? 'bg-blue-100 text-blue-700' :
                                'bg-green-100 text-green-700'
                              }`}>
                                {r.goal}
                              </span>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                r.difficulty === 'Beginner' ? 'bg-emerald-100 text-emerald-700' :
                                r.difficulty === 'Intermediate' ? 'bg-yellow-100 text-yellow-700' :
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                          {(r.days || []).slice(0, 2).map((d, i) => (
                            <div key={i} className="bg-gradient-to-br from-teal-50 to-teal-100 border border-teal-200/50 rounded-xl p-3">
                              ...
                              <div className="space-y-1">
                                {(d.exercises || []).slice(0, 3).map((ex, j) => (
                                  <div key={j} className="flex items-center justify-between text-xs text-muted-foreground">
                                    ...
                                    <span className="text-teal-600 font-medium">{ex.sets}×{ex.reps}</span>
                                  </div>
                                ))}
                                {d.exercises?.length > 3 && (
                                  <div className="text-xs text-muted-foreground">+{d.exercises.length - 3} more</div>
                                )}
                              </div>
                            </div>
                          ))}
                      ...
                      <div className="flex flex-col gap-2">
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={() => { setSelectedRoutine(r); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => deleteRoutine(r.id || r._id)}
                    ...
                    <div className="flex items-center justify-between mt-6 pt-4 border-t border-teal-200/50">
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {r.days?.reduce((total, day) => total + (day.exercises?.length || 0), 0) || 0} exercises
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => { navigator.clipboard.writeText(JSON.stringify(r)); alert('Copied JSON to clipboard'); }}
            ...
            {!loading && filtered.length === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground mb-6">Try adjusting your filters or create your first workout routine.</p>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={openCreate}
          ...
          <aside className="space-y-6">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-bold text-foreground">Routine Details</h4>
              </div>
              {!selectedRoutine ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">Select a routine to view full breakdown</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="bg-gradient-to-br from-teal-50 to-teal-100 border border-teal-200/50 rounded-xl p-4">
                    <h5 className="font-semibold text-lg text-foreground mb-2">{selectedRoutine.name}</h5>
                    <div className="flex items-center gap-2 mb-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        selectedRoutine.goal === 'Strength' ? 'bg-red-100 text-red-700' :
                        selectedRoutine.goal === 'Muscle' ? 'bg-blue-100 text-blue-700' :
                        'bg-green-100 text-green-700'
                      }`}>
                        {selectedRoutine.goal}
                      </span>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        selectedRoutine.difficulty === 'Beginner' ? 'bg-emerald-100 text-emerald-700' :
                        selectedRoutine.difficulty === 'Intermediate' ? 'bg-yellow-100 text-yellow-700' :
                        'bg-red-100 text-red-700'
                      }`}>
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          {(d.exercises || []).map((ex, j) => (
                            <div key={j} className="flex items-center justify-between bg-teal-50 rounded-lg p-2">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-bold text-foreground">Quick Stats</h4>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gradient-to-br from-teal-50 to-teal-100 border border-teal-200/50 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-teal-600 mb-1">{routines.length}</div>
                  <div className="text-sm text-muted-foreground">Total Routines</div>
                </div>
                <div className="bg-gradient-to-br from-teal-50 to-teal-100 border border-teal-200/50 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-teal-600 mb-1">
                    {routines.length ? (routines.reduce((s, r) => s + (r.days?.length || 0), 0) / routines.length).toFixed(1) : 0}
                  </div>
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-bold text-foreground">Pro Tips</h4>
              </div>
              <ul className="space-y-3 text-sm text-muted-foreground">
                <li className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-teal-500 rounded-full mt-2 flex-shrink-0"></div>
                  <span>Keep routines focused: 3–5 prime movements per day for optimal results.</span>
                </li>
                <li className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-teal-500 rounded-full mt-2 flex-shrink-0"></div>
                  <span>Use progressive overload — track weight & reps for member progress.</span>
                </li>
                <li className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-teal-500 rounded-full mt-2 flex-shrink-0"></div>
                  <span>Include warm-up & mobility work on heavy training days.</span>
                </li>
                <li className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-teal-500 rounded-full mt-2 flex-shrink-0"></div>
                  <span>Balance push/pull movements to prevent imbalances.</span>
                </li>
              </ul>
            </motion.div>
          </aside>
        ...
        {formOpen && (
          ...
          >
            <motion.form
              ...
              className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl"
            >
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-2xl font-bold text-foreground">{editing ? "Edit Routine" : "Create Routine"}</h3>
                <button type="button" onClick={() => setFormOpen(false)} className="text-slate-500 hover:text-slate-700">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="mt-6 flex justify-end gap-2">
                <button type="button" onClick={() => setFormOpen(false)} className="px-6 py-2 rounded-xl border border-gray-300 hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={submitting} className="px-6 py-2 rounded-xl bg-teal-600 text-white hover:bg-teal-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
